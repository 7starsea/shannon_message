...
#include "demo-server-forwarders-quick.h"
#include "demo-message-datastruct.h"

void send_data_loop(TestFirstQuickForwarder * fst_forwarder , TestSecondQuickForwarder * snd_forwarder ){
...
int main(){
	...
	TestFirstQuickForwarder fst_forward_quick(messageLoop, ids);
	messageLoop.subscribe	= 211;
	messageLoop.unsubscribe = 212;
	messageLoop.update		= 213;
	TestSecondQuickForwarder snd_forwarder_quick(messageLoop, ids);
	boost::thread tThread(  boost::bind( &send_data_loop, &fst_forward_quick, &snd_forwarder_quick)  );
	tThread.detach();
	...
	server.Register<Shannon::Message::SubscribeField>( &fst_forward_quick );
	server.Register<Shannon::Message::SubscribeField>( &snd_forwarder_quick );
